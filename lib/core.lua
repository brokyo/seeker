...
function update_chord_options()
  ...
  for arp_idx = 1, 2 do
    ...
    params:set('chord_type_' .. arp_idx, 1, 0)
...
function update_chords_for_root(arp_idx)
  local new_root = params:get('chord_root_note_' .. arp_idx) 
  new_root = chord_roots.number[new_root]

  local seeker_key = params:get("seeker_key") - 1
  local seeker_scale = params:string("seeker_scale")

  -- Get available chords for the new root
  local new_chord_types = musicutil.chord_types_for_note(new_root, seeker_key, seeker_scale)

  -- Change param to use new chords
  local chord_types = params:lookup_param('chord_type_' .. arp_idx)
  chord_types.options = new_chord_types
  chord_types.count = #new_chord_types
  params:set('chord_type_' .. arp_idx, 1)
...
local function advance_arp(idx)
  ...
  if math.random() <= probability then
    if player and note then
      ...
      print("Error: Invalid player or note for arp " .. idx)
    end
  end
  ...
  local next_step
  if #chord == 0 then
    -- If the chord is empty, don't try to advance
    next_step = 1
  else
    if style == 1 then
...
function script_api:add_activation_switch()
  ...
  for arp_idx = 1, 2 do
    ...
    params:set_action('chord_root_note_' .. arp_idx, function(param)
      update_chords_for_root(arp_idx)
    ...
    params:add_option("velocity_curve_" .. arp_idx, "Velocity Curve", {"Alternate", "Ramp Up", "Ramp Down", "Sine", "Flat"}, 2)
    ...
    params:add_option("duration_mode_" .. arp_idx, "Duration Control", {"Gate Length", "Trigger", "Dice"}, 2)
    ...
    for lane_idx = 1, 4 do
      ...
      params:set_action("sequenced_param_lane_" .. lane_idx .. "_arp_" .. arp_idx, function(param_idx)

        local param_id = available_params[param_idx]
        local param_meta = params:lookup_param(param_id)
        if param_meta.options then
          for step_idx = 1, 6 do
            local this_step = params:lookup_param("step_" .. step_idx .. "_lane_" .. lane_idx .. "_arp_" .. arp_idx)
          ...
          for step_idx = 1, 6 do
            local this_step = params:lookup_param("step_" .. step_idx .. "_lane_" .. lane_idx .. "_arp_" .. arp_idx)
            local values = {}
            for i = param_meta.min, param_meta.max do
      ...
      params:add_number("step_count_lane_" .. lane_idx .. "_arp_" .. arp_idx, "Step Count", 0, 6, 0)
      
      local nb_voices = params:lookup_param("seeker_voice_1").options
      for step_idx = 1, 6 do
        params:add_option("step_" .. step_idx .. "_lane_" .. lane_idx .. "_arp_" .. arp_idx, "Step " .. step_idx, nb_voices, 1)
      end
...
function increment_paramquencer_step(arp_idx)
  -- print("Starting increment_paramquencer_step for arp_idx:", arp_idx)
  for lane_idx = 1, 4 do
    -- print("Processing lane_idx:", lane_idx)

    local step_count = params:get("step_count_lane_" .. lane_idx .. "_arp_" .. arp_idx)
    -- print("Step count for lane", lane_idx, ":", step_count)
    
    if step_count == 0 then
      -- print("Skipping lane", lane_idx, "due to zero step count")
    else
      ...
      -- print("Current pulse for lane", lane_idx, ":", arp[arp_idx].paramquencer_current_pulse[lane_idx])
      -- print("Current step for lane", lane_idx, ":", arp[arp_idx].paramquencer_current_step[lane_idx])

      if arp[arp_idx].paramquencer_current_pulse[lane_idx] == 1 then
        arp[arp_idx].paramquencer_current_step[lane_idx] = (arp[arp_idx].paramquencer_current_step[lane_idx] % step_count) + 1
        ...
        -- print("Updating param:", param_to_update, "to value:", next_step_value)
        params:set(param_to_update, next_step_value)
        _menu.rebuild_params()
      end
    end
  end
  -- print("Finished increment_paramquencer_step for arp_idx:", arp_idx)
end
